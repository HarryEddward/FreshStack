...
export async function authHandler(
  ...
  try {
    ...
    const licenseUrl = `/${DEFAULT_LANG}/business/web/app`;
...
// ✅ Corrección: esto devuelve una función que toma (req, ctx) y retorna Promise<Response>
export function uniqueAuthHandler(
  fn: (req: Request, ctx: FreshContext<State>) => Promise<Response>
): (req: Request, ctx: FreshContext<State>) => Promise<Response> {
  return async (req: Request, ctx: FreshContext<State>): Promise<Response> => {
    return await authHandler(req, ctx, async () => await fn(req, ctx));
  };
}
